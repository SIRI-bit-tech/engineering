...
export const Navbar = () => {
  ...
  return (
    <>
      <header
        ref={headerRef}
        className={`fixed top-0 left-0 w-full z-1000 transition-all duration-500 ${isMobileMenuOpen ? "bg-[#00253B] border-b border-white/10" : "bg-white/70 backdrop-blur-xl border-b border-primary/5"
          }`}
      >
        <nav className="container-wide h-24 flex items-center justify-between relative z-1001">
          ...
          {/* Mobile Menu Toggle */}
          <button
            className={`lg:hidden p-2 rounded-xl transition-all duration-300 border ${isMobileMenuOpen ? "text-white bg-white/20 border-white/20" : "text-primary hover:bg-ocean/10 border-transparent"}`}
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 h-screen w-screen bg-[#00253B] z-999 lg:hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen ? "translate-x-0 opacity-100 pointer-events-auto" : "translate-x-full opacity-0 pointer-events-none"
          }`}
      >
        ...
        <ul className="relative z-1000 flex flex-col p-8 pt-32 space-y-8 h-full overflow-y-auto">
          {NAV_LINKS.map((link) => (
          ...
          <li className="pt-8 mt-auto pb-24">
